import 'package:chat_app_firebase/domain/chat/chat.dart';
...
final chatStreamProvider = StreamProvider.autoDispose<List<Chat>>((ref) {
  ...
  Stream<QuerySnapshot<Map<String, dynamic>>> stream = db.collection("chatrooms/m2uAeVyGwziHhkDRZwNZ/chat").snapshots(); // map타입 스트림 - 사진찍을건데 타입을 모르니까 map 타입
...
class ChatFireStoreRepository {
  ...
  Future<DocumentReference> insert(ChatInsertReqDto dto) {
    // 문서로 돌려받음
    return db.collection("chatrooms/m2uAeVyGwziHhkDRZwNZ/chat").add(dto.toJson());
    // db에 insert
  }
}
